...
function addNewValueToBoard () {
    if (board.indexOf(0) >= 0) {
        blankBoardLocations = []
        for (let index = 0; index <= BOARD_SIZE * BOARD_SIZE - 1; index++) {
            if (board[index] == 0) {
                blankBoardLocations.push(index)
            }
        }
...
function updateBoard () {
    for (let index = 0; index <= BOARD_SIZE * BOARD_SIZE - 1; index++) {
        blocks.place(blocks.blockByName("element_1" + board[index]), positions.add(
        getBoardLocation(index),
        pos(0, 0, -1)
        ))
    }
    blocks.clone(
...
function buildBoard () {
    ...
    for (let index = 0; index <= BOARD_SIZE * BOARD_SIZE - 1; index++) {
        board.push(0)
        blocks.place(blocks.blockByName("element_1"), getBoardLocation(index))
    }
}
function slideRight () {
    ii = BOARD_SIZE - 1
